  ...
  async function handleClick(trip) {
    ...
    console.log(`Trip name: ${trip.tripname}, Trip code: ${trip.tripcode}`);
      ...
      .then((res) => {
        ...
        if (res.data) {
          navigate(`/finaltrip/${trip.tripcode}`, { replace: true , state: { tripData: res.data }  });
        }
      })
      .catch((err) => {
        if (err.response) {
      ...
      <div className="flex justify-center  flex-wrap items-center align-center">
        {trips.map((trip) => (
         <div className='m-4'>
          ...
          <div className=" w-[300px] bg-red rounded-lg shadow-md p-4 text-center" key={trip._id}>
          <button  onClick={() => handleClick(trip)}>
            ...
            <button className=" w-[5rem] bg-[#1a43bf] text-white px-2 py-1 rounded-md hover:bg-pink-700 duration-300" onClick={()=>deletetrip(trip)}>Delete</button>
          <CopyToClipboard text={trip.tripcode} onCopy={handleCopy}>
          <button className=" w-[5rem] bg-[#1a43bf] text-white px-2 py-1 rounded-md hover:bg-pink-700 duration-300">Copy</button>
      </CopyToClipboard>
      </div>
    </div>
